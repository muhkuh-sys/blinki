...
#elif ASIC_TYP==50
static void i2c_hifsta_setPins(unsigned int uiScl, unsigned int uiSda)
{
	unsigned long ulNewClockPart;
	unsigned long ulNewDataPart;


	ulNewClockPart = uiScl<<SRT_sta_netx_rdy_drv;

	// output new value
	ptNetxControlledGlobalRegisterBlock1Area->ulSta_netx = ulLastDataPart|ulNewClockPart;

	ulNewDataPart = uiSda<<SRT_sta_netx_run_drv;

	// output new value
	ptNetxControlledGlobalRegisterBlock1Area->ulSta_netx = ulNewDataPart|ulNewClockPart;

	// remember new data value for next output
	ulLastDataPart = ulNewDataPart;
}
